...
m=$(printf '%s\n' "$REPO_FILES" | xargs -d '\n' grep -inHoEI '[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}' 2>/dev/null | grep -viE "$EMAIL_ALLOW" || true)
[ -n "$m" ] && builtin+="$m"$'\n'
...
m=$(printf '%s\n' "$REPO_FILES" | grep -vE "$IP_EXCLUDE" \
  | xargs -d '\n' grep -inHEI '\b(192\.168|10\.[0-9]{1,3}|172\.(1[6-9]|2[0-9]|3[01])|100\.[0-9]{1,3}|169\.254)\.[0-9]{1,3}\.[0-9]{1,3}\b' 2>/dev/null \
